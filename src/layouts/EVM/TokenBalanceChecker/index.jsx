...
import { getMultiWalletTokenBalance } from 'utils/APIs/MoralisAPI'
  ...
  const checkAccount = async () => {
    ...
    if (address === '' || address === null || address === undefined) {
      ...
      if (String(address).includes(',')) {
        ...
        const data = await getMultiWalletTokenBalance(addressList)
        setBalances(data)
      } else {
        const data = await getMultiWalletTokenBalance(Array(address))
        setBalances(data)
      }
    }
    setIsLoading(false)
          ...
          <Card title='ტოკენების ბალანსის შემოწმება' titleBorder={true}>
            <div className='p-2'>
              <div className='mb-2 bg-darkBackground p-1 rounded-md border-[1px] border-lightBorder'>
                <Typography color='text-gray-500 text-sm'>შეამოწმეთ ერთ ან რამოდენიმე საფულის ტოკენების ბალანსი ერთდროულად EVM ქსელებზე.</Typography>
                      ...
                      <div className='px-2'>
                        {x.data.map((y, index) => (
                          <div key={index} className='mb-2'>
                            <Card title={`${y.chainSymbol} ქსელი - ${y.balance.length} ტოკენი`} variant='collapsible'>
                              <div className=''>
                                  ...
                                  <div>
                                    {y.balance.length > 0 ? (
                                      <Table>
                                        ...
                                        <Tbody>
                                          {y.balance.map((z, index) => (
                                            <BodyTr key={index}>
                                              <BodyTd isLast={index !== y.balance.length - 1} rightCorner={index === y.balance.length - 1}>
                                                <Typography>{index + 1}</Typography>
                                              </BodyTd>
                                              <BodyTd isLast={index !== y.balance.length - 1}>
                                                <div className='flex items-center gap-2'>
                                                  <Avatar src={z.logo || z.thumbnail || ''} alt='test' />
                                              ...
                                              <BodyTd isLast={index !== y.balance.length - 1} leftCorner={index === y.balance.length - 1}>
                                                <Typography>{(Number(z.balance) / 10 ** z.decimals).toLocaleString('en-US')}</Typography>
                                              </BodyTd>
                                            </BodyTr>
                                          ))}
                                        </Tbody>
                                      ...
                                      <div className='p-2'>
                                        <Alert variant='info' text={`${y.chainSymbol} ქსელზე ტოკენები არ არის!`} />
                                      </div>
                                    )}
                                  </div>
                                ) : (
